...
const AdminEditGezi: React.FC = () => {
  ...
  useEffect(() => {
    const savedPosts = JSON.parse(localStorage.getItem('geziPosts') || '[]');
    setPosts(savedPosts);
  }, []);
      ...
      <div className="flex-grow bg-gray-100">
        ...
        <header className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white p-5 shadow-lg">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <h1 className="text-2xl font-bold">Gezi Rehberim İçerik Düzenleme</h1>
          </div>
        </header>

        {/* Editor */}
        <main className="p-8">
          <h2 className="text-2xl text-black font-bold mb-4">Blog İçeriğini Düzenle</h2>
          <input
            type="text"
            value={title}
          ...
          <ul>
            {posts.map((post, index) => (
              post && ( // post nesnesinin null olup olmadığını kontrol et
